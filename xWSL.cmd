...
:APTRELY
START /MIN /WAIT "Find best mirror..." %GO% "MyIP=$(curl -s ifconfig.me) ; MyCountry=$(curl -s ipinfo.io/$MyIP/country) ; echo Region: $MyCountry ; apt-select -C $MyCountry ; mv sources.list /etc/apt/ ; add-apt-repository --yes ppa:oibaf/graphics-drivers 2> /tmp/apterr"
